...
SELECT COMMENT_CODE, MEMBER_NICKNAME, COMMENT_DESC FROM VIDEO_COMMENT JOIN MEMBER USING(MEMBER_ID) WHERE VIDEO_CODE=3;
...
JOIN MEMBER M ON (V.MEMBER_ID = V.MEMBER_ID) WHERE V.VIDEO_CODE=3;
...
-- 좋아요 취소 (deleteLike)
DELETE FROM VIDEO_LIKE WHERE VIDEO_CODE=3 AND MEMBER_ID='user1';
COMMIT;
SELECT * FROM VIDEO_LIKE;

-- 구독 추가(addSubscribe)
INSERT INTO SUBSCRIBE(SUBS_CODE, CHANNEL_CODE, MEMBER_ID) VALUES(SEQ_SUBSCRIBE.NEXTVAL, 1, 'user1');
COMMIT;
...
WHERE S.MEMBER_ID='user1';

-- 구독 취소(deleteSubscribe)
DELETE FROM SUBSCRIBE WHERE CHANNEL_CODE=1 AND MEMBER_ID='user1';
COMMIT;
SELECT * FROM SUBSCRIBE;
