/*
    ...
    : 데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나,
      수정(UPDATE)하거나, 삭제(DELETE)하는 구문이다.
    ...
    2) INSERT INTO 테이블명(컬럼명, 컬럼명, ..., 컬럼명)
                VALUES(값, 값, ..., 값);
        - 테이블에 특정 컬럼에 대한 값만 INSERT 하고자 할 때 사용
...
-- 표현법 3)
-- EMPLOYEE 테이블에서 직원의 사번, 이름, 부서명, 입사일을 조회해서 EMP_01을 INSERT 하시오.
INSERT INTO EMP_01(
...
-- EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서코드가 D1인 직원의 사번, 이름, 부서코드, 입사일을 삽입하고 
...
/*
    ...
    - UPDATE시 서브쿼리를 사용해서 서브 쿼리를 수행한 결과값으로 컬럼값을 변경 할 수 있다.
*/
...
WHERE SALARY = 8000000 AND BONUS = 0.3;
...
-- DEPT_CODE 테이블에서 DEPT_ID가 D1인 부서 삭제 (여기도 다시!)
