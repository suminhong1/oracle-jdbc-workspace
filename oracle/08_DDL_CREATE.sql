/*
    ...
    * 오라클에서 객체(구조) : 테이블(TABLE), 뷰(VIEW), 시퀀스(SEQUENCE), 
    ...
    함수(FUNTION),동의어(SYNONYM),사용자(USER)
*/
/*
    * CREATE
    - 객체를 생성하는 구문
...
INSERT INTO MEM_UNUQUE VALUES(2,'user01','pass02','강개똥', null,null,null);

--> UNIQUE 제약조건에 위배되었으므로 insert 실패!
--> 오류 구문에 제약조건명으로 알려줌!
-- 제약조건 부여서 제약조건명을 지정해주지 않으면 
-- 시스템에서 알아서 임의의 제약조건명을 부여해버림
...
/*
    ...
    * 삭제옵션 : 부모테이블의 데이터 삭제 시 그 데이터를 사용하고 있는
    ...
    - ON DELETE SET NULL
        : 부모 데이터 삭제 시 해당 데이터를 쓰고 있는 자식데이터의 값을 NULL로 변경
    - ON DELETE CASCADE
        : 부모 데이터 삭제 시 해당 데이터를 쓰고 있는 자식데이터도 같이 삭제시킴
*/
DROP TABLE MEM;
CREATE TABLE MEM(
    MEM_NO NUMBER PRIMARY KEY,
...
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    FROM EMPLOYEE
    WHERE 1 = 0; --> 구조만 복사하고자 할 때 쓰이는 구문
...
--04. 도서를 대여한 회원에 대한 데이터를 담기 위한 대여 목록 테이블
...
CREATE TABLE TB_RENT(
    ...
    CONSTRAINT TB_RENT_MEM_NO_FK FOREIGN KEY(RENT_MEM_NO) REFERENCES TB_MEMBER ON DELETE SET NULL,
    CONSTRAINT TB_RENT_BOOK_NO_FK FOREIGN KEY(RENT_BOOK_NO) REFERENCES TB_BOOK ON DELETE SET NULL
);
