...
SELECT EMAIL ,INSTR(EMAIL,'@') "@ 위치",INSTR(EMAIL,'s',-1,2) "'s'뒤에서 2번째 위치"
FROM EMPLOYEE
WHERE EMAIL LIKE '%s%';
...
SELECT EMP_NAME , EMAIL ,REPLACE(EMAIL, 'kh.or.kr','gmail.com') FROM EMPLOYEE;
...
-- 직원명, 입사일, 근무 개월 수 조회
SELECT EMP_NAME, HIRE_DATE, CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE))"근무 개월 수" FROM EMPLOYEE;
...
SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(HIRE_DATE, 6) "입사 6개월 후" FROM EMPLOYEE;

/*
    NEXT_DAY(DATE, 요일(문자|숫자)
    - 특정 날짜에서 구하려는 요일에 가장 가까운 날짜를 리턴
...
-- 직원명, 입사일, 입사월의 마지막 날짜, 입사한 월에 근무한 일수조회
SELECT EMP_NAME,HIRE_DATE,LAST_DAY(HIRE_DATE), LAST_DAY(HIRE_DATE)-HIRE_DATE + 1 "일수" FROM EMPLOYEE;

/*
    EXTRACT(YEAR|MONTH|DAY FROM DATE)
    - 특정 날짜에서 연도, 월, 일 정보를 추출해서반환
        YEAR : 연도만 추출
        MONTH : 월만 추출
        DAY : 일만 추출
*/
...
-- 문자--> 날짜
 ...
 SELECT TO_DATE('2023021 033823', 'YYYYMMDD HH24MISS') FROM DUAL;
...
-- 사원명, 보너스 조회
SELECT 
EMP_NAME, NVL(BONUS, 0)
FROM EMPLOYEE;

-- 사원명, 보너스포함연봉(월급 + 월급*보너스) * 12 조회
SELECT EMP_NAME ,TO_CHAR((SALARY + SALARY * NVL(BONUS,0))* 12,'L999,999,999') "연봉"
FROM EMPLOYEE;
...
SELECT TO_CHAR(SUM(SALARY*12),'FM999,999,999') "연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
...
MAX(EMP_NAME),MAX(SALARY),MAX(HIRE_DATE) FROM EMPLOYEE;
