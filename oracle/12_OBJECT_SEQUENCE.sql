...
/*
    ...
    [NOACHE|CACHE 바이트크기]     --> 캐시메모리 할당 (기본값 CACHE 20)
    
    * 캐시메모리 
        - 미리 발생된 값들을 생성해서 저장해두는 공간
        - 매번 호출될 때마다 새로이 번호를 생성하는게 아니라
        캐시매모리 공간에 미리 생성된 값들을 가져다 쓸 수 있음 (속도가 빨라짐)
...
*/
--NEXTVAL를 한 번이라도 수행하지 않은 이상 CURRVAL를 가져올 수 없음!
...
/*
    ...
    [CYCLE|NOCYCLE] --> 값 순환 여부 지정 (기본값 NOCYCLE)
    [NOCACHE|CACHE 바이트크기] --> 캐시메모리 할당(기본값CACHE 20)
