...
SELECT EMP_NAME, HIRE_DATE, CEIL(SYSDATE - HIRE_DATE) ,SYSDATE
FROM EMPLOYEE;
...
/*
    ...
    - 산술 연산을 하게 되면 컬럼명이 지저분해진다. 이때 컬럼명에 별칭을 부여해서 깔끔하게 보여줄 수 있다.
...
-- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사원명, 급여, 보너스포함연봉 조회 (별칭)
...
/*
    ...
    - 실행순서 : FROM절 -> WHERE 절 -> SELECT 절 -> ORDER BY 절
    
    [표현법]
    SELECT 컬럼, 컬럼, ...
    FROM 테이블명
    WHERE 조건식
    ORDER BY 정령하고자하는컬럼값 [ASC|DESC] [NULLS FIRST|NULLS LAST];
