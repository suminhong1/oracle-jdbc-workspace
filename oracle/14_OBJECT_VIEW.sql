...
-- 사원의 사번, 사번명, 직급명, 성별(남/여), 근무년수를 조회할수 있는 SELECT문을 뷰(VW_EMP_JOB)로 정의
SELECT EMP_ID, JOB_NAME, DECODE(SUBSTR(EMP_NO,8,1),1,'남',2,'여'),EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE)
FROM EMPLOYEE E
JOIN JOB J USING(JOB_CODE);

-- 1) 서브쿼리에서 별칭을 부여하는 방법
CREATE OR REPLACE VIEW VW_EMP_JOB
AS SELECT EMP_ID, JOB_NAME, DECODE(SUBSTR(EMP_NO,8,1),1,'남',2,'여')"성별",EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM HIRE_DATE)"근무년수"
FROM EMPLOYEE E
JOIN JOB J USING(JOB_CODE);
...
CREATE OR REPLACE VIEW VW_GROUPDEPT("부서코드","합계","평균")
AS SELECT  DEPT_CODE, SUM(SALARY), FLOOR(AVG(NVL(SALARY,0)))
FROM EMPLOYEE
GROUP BY DEPT_CODE;
...
-- VIEW 옵션
-- 2. FORCE | NOFORCE
-- NOFORCE (기본값) : 서브쿼리에 기술된 테이블이 존재하는 테이블이어만 뷰가 생성되게함
DROP TABLE TEST;
