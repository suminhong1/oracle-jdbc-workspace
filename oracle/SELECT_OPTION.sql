...
SELECT STUDENT_NO, STUDENT_NAME, DEPARTMENT_NAME, AVG(NVL(POINT))
FROM TB_STUDENT S
JOIN TB_DEPARTMENT D USING(DEPARTMENT_NO)
JOIN TB_GRADE G USING(STUDENT_NO)
WHERE ABSENCE_YN = 'N';
--15. 환경조경학과 전공과목들의 과목 별 평점을 파악할 수 있는 SQL 문을 작성하시오. 단, 평점은
...
SELECT SUM(POINT), STUDENT_NAME, DEPARTMENT_NAME
FROM TB_GRADE G
