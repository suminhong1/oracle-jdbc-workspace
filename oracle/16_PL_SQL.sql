/*
    ...
    * PL/SQL 구조
    - [선언부(DECLARE SECTION)] : DECLARE로 시작, 
                                     변수나 상수를 선언 및 초기화하는 부분
    - 실행부 (EXECUTABLE SECTION) : BEGIN으로 시작, 
                SQL문 또는 제어문(조건문, 반복문) 등의 로직을 기술하는 부분
    - [예외처리부(EXCEPTION SECTION)] : EXCEPTION으로 시작, 
                 예외발생시 해결하기 위한 구문을 미리 기술해둘 수 있는 부분
*/
-- 출력 기능 활성화
SET SERVEROUTPUT ON;
...
-- (부서의 변수명은 DNAME, 데이터타입은 VARCHAR2(30))
...
/*
    ...
    * 오라클에서 미리 정의되어 있는 시스템 예외
    ...
    - TOO_MANY_ROWS : SELECT한 결과가 한 행이 리턴되어야 하는데
                                                       여러 행일 경우
