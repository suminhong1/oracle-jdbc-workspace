/*
    ...
    - 테이블이나 뷰가 DML(INSERT, UPDATE, DELETE)문에 의해 변경될 경우
      ...
      * 트리거 종류
      ...
      > ROW TRIGGER(행 트리거) : 해당 SQL문 실행할 때마다 매번 트리거 실행(FOR EACH ROW 옵션 기술해야됨)
...
VALUES(500,'박진실','111111-222222');

INSERT INTO EMPLOYEE(EMP_ID,EMP_NAME,EMP_NO)
VALUES(501, '이창희', '222222-111111');

-- 2. 행 트리거
-- :OLD : 수정, 삭제 전 데이터에 접근 가능
-- :NEW : 추가, 수정 후 데이터에 접근 가능
-- EMPLOYEE 테이블에 UPDATE 수행 후 메시지를 자동으로 출력
