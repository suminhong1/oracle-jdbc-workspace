 /*
    ...
    - 관계형 데이터 베이스는 최소한의 데이터로 각각의 테이블에 담고 있음
     ...
     - 크게 "오라클 구문"과 "ANSI 구문"
     - ANSI(미국국립표준협회 == 산업표준을 제정하는 단체) : 다른 DBMS 에서도 사용이 가능
 */
 ...
 */
 ...
 FROM EMPLOYEE 
 JOIN DEPARTMENT ON (DEPT_ID = DEPT_CODE) AND NOT DEPT_TITLE = '총무부';
...
SELECT EMP_NAME, DEPT_TITLE, SALARY, SALARY*12
FROM EMPLOYEE,DEPARTMENT
WHERE DEPT_CODE = DEPT_ID (+); -- 기준으로 삼고자하는 테이블의 반대편
                               -- 테이블의 컬럼 뒤에 (+) 붙이기
...
/*
    ...
    ANSI 구문으로는 JOIN ON으로만 사용 가능! (USING 사용 불가)
*/
...
-- 사원 사번, 사원명, 사원부서코드, 사수사번, 사수명, 사수부서코드 조회
-- >> 오라클 구문
...
WHERE D.DEPT_ID = E.DEPT_CODE
    ...
    AND EMP_NAME LIKE '전%';
-- 3. 보너스를 받는 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역을 조회 단, 부서코드가 없는 사원도 출력될 수 있게!
...
FROM DEPARTMENT D, EMPLOYEE E, LOCATION L, NATIONAL N
WHERE D.DEPT_ID = E.DEPT_CODE
AND D.LOCATION_ID = L.LOCAL_CODE
AND L.NATIONAL_CODE = N.NATIONAL_CODE
AND NATIONAL_NAME IN ('한국','일본') ;
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여(정수처리)를 조회
...
AND D.LOCATION_ID = L.LOCAL_CODE;
-- 10. 해외영업팀에 근무하는 직원들의 직원명, 직급명, 부서 코드, 부서명 조회
