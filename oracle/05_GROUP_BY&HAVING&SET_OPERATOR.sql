...
-- 컬럼이 하나일 때는 사실상 CUBE, ROLLUP이 차이점이 딱히 없음!
...
-- 위 커리문 대신 WHERE절에 OR 연산자를 사용해서 처리 가능
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
...
-- 4. MINUS
-- 부서 코드가 D5인 사원들 중에서 급여가 300만원 초과인 사람들을 제외해서 조회
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5'
MINUS
...
-- 위 쿼리문 대신에 WHERE절에 AND 연산자를 사용해서 처리 가능
SELECT EMP_ID,EMP_NAME,DEPT_CODE,SALARY
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' AND SALARY <= 3000000;
